...
float in_shadow(vec4 light_space_frag_pos);
...
void main() {
    ...
    float shadow = in_shadow(light_space_frag_pos);
    frag_color = vec4(color_output, 1.0) * (1.0 - shadow);
}
...
float in_shadow(vec4 light_space_frag_pos) {
    ...
    float bias = max(0.05 * (1.0 - dot(normalize(normal), normalize(point_lights[0].position - fragment_position))), 0.005); 
    float shadow = 0.0;
